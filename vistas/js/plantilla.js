document.addEventListener("DOMContentLoaded", function () {
  ...
  const celdasBotones = document.querySelectorAll(
    ".td-pending, .td-missing, .td-approved, .td-rejected",
  );
  ...
  celdasBotones.forEach((celda) => {
    ...
    celda.addEventListener("click", function () {
      ...
      console.log(
        `Documento: ${documento}, Nombre: ${nombre}, Estado clicado: ${estadoActual}`,
      );
  ...
  if (btnExcel) {
    btnExcel.addEventListener("click", function () {
      ...
      celdas.forEach((td) => {
        ...
        if (
          ...
          document.querySelectorAll(
            ".td-pending, .td-approved, .td-rejected, .td-missing",
          ).length > 0
        ) {
      ...
      let blob = new Blob([contenidoBase], {
